...
import { heartbeatSession, isIclinicaSyncConfigured, releasePlantaoSession } from "../services/iclinicaSync.js";
...
sessionsRouter.post("/sessions/release-plantao", async (req, res) => {
  try {
    const tenantSlug = resolveTenantSlug(req.headers["x-tenant-slug"]);
    const body = req.body as Record<string, unknown>;
    const sessionToken = typeof body.sessionToken === "string" ? body.sessionToken.trim() : "";
    ...
    await releasePlantaoSession(tenant.slug, sessionToken);
    return res.json({ ok: true });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Falha ao sair da fila.";
    return res.status(500).json({ error: "release_plantao_failed", message });
  }
});
